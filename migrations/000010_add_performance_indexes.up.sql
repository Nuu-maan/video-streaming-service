...
CREATE INDEX IF NOT EXISTS idx_videos_visibility_status 
ON videos(visibility, status);

CREATE INDEX IF NOT EXISTS idx_videos_status_created 
ON videos(status, created_at DESC) WHERE visibility = 'public';

CREATE INDEX IF NOT EXISTS idx_videos_view_count_desc 
ON videos(view_count DESC) WHERE status = 'ready' AND visibility = 'public';

CREATE INDEX IF NOT EXISTS idx_videos_like_count_desc 
ON videos(like_count DESC) WHERE status = 'ready' AND visibility = 'public';

CREATE INDEX IF NOT EXISTS idx_videos_created_status_visibility 
ON videos(created_at DESC, status, visibility);
...
CREATE INDEX IF NOT EXISTS idx_comments_video_likes 
...
CREATE INDEX IF NOT EXISTS idx_watch_history_video_count 
...
CREATE INDEX IF NOT EXISTS idx_subscriptions_creator_created 
ON subscriptions(creator_id, created_at DESC);

-- Likes performance indexes  
CREATE INDEX IF NOT EXISTS idx_likes_video_is_like 
ON likes(video_id, is_like) WHERE is_like = true;

-- Playlists performance indexes
CREATE INDEX IF NOT EXISTS idx_playlists_user_visibility 
...
CREATE INDEX IF NOT EXISTS idx_audit_logs_target_type_created 
...
ON videos(id, created_at) WHERE status = 'uploading';

-- Covering indexes for common read patterns
CREATE INDEX IF NOT EXISTS idx_videos_list_covering 
